...
- **`node_generic_sql.py`** - Generates custom SQL queries using LLM and database schema when user opts for generic path. Executes query and returns results, bypassing predefined scenarios.
...
### Using Generic SQL (Medium-Certainty Queries)

When the system has medium certainty (2-7) about your query classification, you'll see three options:
...
**Option 2: Generic SQL** allows the assistant to:
- Analyze your question and conversation history
- Use the database schema to generate a custom SQL query
- Execute the query and provide results
- Skip predefined scenarios when they don't quite fit

**When to use Generic SQL:**
...
- Confirmation flow for medium-certainty matches (2-7)
...
- Three-option confirmation: (1) Confirm scenario, (2) Generate custom SQL, (3) Reclassify
